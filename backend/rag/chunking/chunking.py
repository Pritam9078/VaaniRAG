...
def fixed_size_chunks(doc_id: str, text: str, language: str,
                       size: int = 60, overlap: int = 15, is_selected: int = 0) -> list[Chunk]:
    words = text.split()
    if not words:
        return []
    chunks = []
...
def sentence_window_chunks(doc_id: str, text: str, language: str,
                            window: int = 3, overlap: int = 1, is_selected: int = 0) -> list[Chunk]:
...
def semantic_chunks(doc_id: str, text: str, language: str,
                     sim_threshold: float = 0.15, max_sentences: int = 6, is_selected: int = 0) -> list[Chunk]:
    sents = split_sentences(text)
    if len(sents) <= 1:
        return sentence_window_chunks(doc_id, text, language, window=len(sents) or 1, is_selected=is_selected)

    vec = TfidfVectorizer().fit(sents)
    X = vec.transform(sents).toarray()  # type: ignore
...
def adaptive_chunks(doc_id: str, text: str, language: str,
                     base_window: int = 3, is_selected: int = 0) -> list[Chunk]:
    sents = split_sentences(text)
    if not sents:
        return []
...
def chunk_document(doc_id: str, text: str, language: str,
                    strategies: list[str] | None = None, is_selected: int = 0) -> list[Chunk]:
    ...
    for name in strategies:
        fn = STRATEGIES[name]
        out.extend(fn(doc_id, text, language, is_selected=is_selected))
    return out
