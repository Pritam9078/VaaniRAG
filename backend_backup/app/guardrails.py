...
RELEVANCE_THRESHOLD = 0.05          # min top-chunk relevance_score to attempt an answer
GROUNDING_THRESHOLD = 0.12          # min lexical overlap between answer and context
...
def check_output_grounding(answer: str, context_chunks: list[dict[str, Any]]) -> GuardrailResult:
    ...
    if overlap < GROUNDING_THRESHOLD:
        return GuardrailResult(
            False, "output",
            f"Answer grounding ({overlap:.2f}) below threshold "
            f"({GROUNDING_THRESHOLD}); possible hallucination.",
        )
